...
uint8_t USART_RX_BUF[200];     //接收缓冲,最大200个字节.
...
uint8_t comGetChar(COM_PORT_E _ucPort, uint8_t *_pByte)
{
	...
	pUart = ComToUart(_ucPort); //功能说明: 将COM端口号转换为UART指针
	if (pUart == 0)
	{
		return 0;
	}
...
static uint8_t UartGetChar(UART_T *_pUart, uint8_t *_pByte)
{
	...
	if (usCount == 0)	/* 已经没有数据 */  //实质是判断_pUart->usRxCount是多少
	{
		return 0;
	}
	else
...
static void UartIRQ(UART_T *_pUart)
{
	...
	if (USART_GetITStatus(_pUart->uart, USART_IT_RXNE) != RESET)
	{
		...
		//以下代码移植正点原子  0x0d 0x0a 没有算到USART_RX_STA
		//同时接收到的数据，存到数组_pUart->pRxBuf[_pUart->usRxWrite] 
		//所以删去了源代码中储存的部分代码
		if((USART_RX_STA & 0x8000) == 0) //还未接收0X0a 
		{
			if(USART_RX_STA&0x4000)//接收到了0x0d
			{
				if(ch!=0x0a)USART_RX_STA=0;//接收错误,重新开始
		...
		{
			_pUart->usRxWrite = 0;   //越界后，_pUart->usRxWrite = 0
		}
		if (_pUart->usRxCount < _pUart->usRxBufSize)
		{
			_pUart->usRxCount++;     //计算读出到的数据个数
		}
